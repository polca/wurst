...
def read_pfc_emissions(scenario_base, gas):
    ...
    """
    assert isinstance(scenario_base, Path), "Must pass a ``pathlib.Path`` as ``scenario_base"
    ...
    as_dict = {year: {a: b for a, b in zip(REGIONS, image_output.data[:, col_index, year_index])}
...
def get_coal_biosphere_exchanges_to_be_scaled_by_efficiency(ds):
    exclude_list = ['Methane, fossil', 'Sulfur dioxide',
        'Carbon monoxide, fossil', 'Nitrogen oxides', 'Dinitrogen monoxide',
        'Particulates']
