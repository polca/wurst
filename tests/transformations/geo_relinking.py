...
def test_relink_non_exclusive():
    ...
    expected = {
        ...
        'exchanges': [{
            'name': 'A', 'product': 'B', 'unit': 'C',
            'amount': 7.5, 'loc': 7.5, 'uncertainty type': 0,
            'location': 'NO',
            'type': 'technosphere',
        }, {
            'name': 'A', 'product': 'B', 'unit': 'C',
            'amount': 2.5, 'loc': 2.5, 'uncertainty type': 0,
            'location': ('ecoinvent', 'UN-NEUROPE'),
            'type': 'technosphere',
        }]
    }
...
def test_relink_intersects(defaults):
    ...
    assert relink_technosphere_exchanges(ds, data, exclusive=False, contained=False) == expected
